...
G_DEFINE_TYPE_WITH_CODE(GstSRTClientSrc, gst_srt_client_src,
                        GST_TYPE_SRT_BASE_SRC, G_ADD_PRIVATE(GstSRTClientSrc) GST_DEBUG_CATEGORY_INIT(GST_CAT_DEFAULT, "srtclientsrc", 0, "SRT Client Source"));
...
gst_srt_client_src_fill(GstPushSrc *src, GstBuffer *outbuf)
{
  ...
  if (srt_epoll_wait(priv->poll_id, 0, 0, ready, &(int){2}, priv->poll_timeout, 0, 0, 0, 0) == -1)
  {
...
static gboolean gst_srt_client_src_stop(GstBaseSrc *src)
{
  GstSRTClientSrc *self = GST_SRT_CLIENT_SRC(src);
  GstSRTClientSrcPrivate *priv = GST_SRT_CLIENT_SRC_GET_PRIVATE(self);
  if (priv->poll_id != SRT_ERROR)
...
static void gst_srt_client_src_class_init(GstSRTClientSrcClass *klass)
{
  ...
  properties[PROP_RENDEZ_VOUS] =
      g_param_spec_boolean("rendez-vous", "Rendez Vous",
                           ...
                           G_PARAM_READWRITE | GST_PARAM_MUTABLE_READY |
                               G_PARAM_STATIC_STRINGS);
  g_object_class_install_properties(gobject_class, PROP_LAST,
                                    properties);
  gst_element_class_add_static_pad_template(gstelement_class,
                                            &src_template);
  gst_element_class_set_metadata(gstelement_class, "SRT client source",
                                 "Source/Network", "Receive data over the network via SRT",
...
static void gst_srt_client_src_init(GstSRTClientSrc *self)
{
  GstSRTClientSrcPrivate *priv = GST_SRT_CLIENT_SRC_GET_PRIVATE(self);
  priv->sock = SRT_INVALID_SOCK;
  priv->poll_id = SRT_ERROR;
